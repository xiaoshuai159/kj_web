...
export default {
    ...
    created() {
        ...
        if (this.$route.query.from == 'update' && this.id != 0) { //获取浏览器url的参数，from页面来源: table客户端列表页面，update更新客户端信息页面 
            ...
            this.getStandardizaById(this.id);
        }
        this.getAllList();
    ...
    methods: {
        ...
        //按id查询
        async getStandardizaById(id) {
            console.log('被调用getStandardizaById');
            let list = {
                id: id
            }
            const { data: res } = await this.$http.post('/api/standardiza/getStandardizaById', list);
            if (res.code == 200) {
        ...
        async loadAPI() {
            ...
            if (this.$route.query.from == 'update') {
                const { date: res } = await this.$http.post('/api/standardiza/updateStandardiza', list);
                if (res.code == 200) {
                    this.$message({
                        message: "更新成功",
                        type: "success",
                ...
                const { date: res } = await this.$http.post('/api/standardiza/addStandardiza', list);
                if (res.code == 200) {
                    this.$message({
                        message: "添加成功",
                        type: "success",
